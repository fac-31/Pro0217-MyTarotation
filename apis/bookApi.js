...
export async function getBook(recommend) {
  try {
    ...
    let bookDataResponse = await fetch(
    ...
    let description = blurbData?.description?.value ?? blurbData?.description ??
      "No description available.";

    let genres =
      Array.isArray(blurbData?.subjects) && blurbData.subjects.length > 0
